...
@implementation FlutterGPUImagePicture
- (id)initWithImage:(UIImage *)newImageSource outPutSize:(CGSize)outPutSize {
    
    return [self initWithCGImage:[newImageSource CGImage] smoothlyScaleOutput:YES removePremultiplication:NO outPutSize:outPutSize];
}

- (id)initWithCGImage:(CGImageRef)newImageSource smoothlyScaleOutput:(BOOL)smoothlyScaleOutput removePremultiplication:(BOOL)removePremultiplication outPutSize:(CGSize)outPutSize;
{
    if (!(self = [super init]))
    {
        return nil;
...
- (BOOL)processImageWithCompletionHandler:(void (^)(void))completion;
{
    ...
    runAsynchronouslyOnVideoProcessingQueue(^{
        ...
        dispatch_semaphore_signal(self->imageUpdateSemaphore);
